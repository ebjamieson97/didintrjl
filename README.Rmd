...
**didintrjl** is an R wrapper for the Julia package [DiDInt.jl](https://ebjamieson97.github.io/DiDInt.jl/stable/), which implements intersection difference-in-differences (DID-INT),
...
The stable [CRAN](https://cran.r-project.org/package=didintrjl) version of didintrjl can be installed via:
...
Since **didintrjl** is a wrapper for the Julia pacakage [DiDInt.jl](https://ebjamieson97.github.io/DiDInt.jl/dev/), you will need to ensure that **DiDInt.jl** is installed and you may need to update the Julia package itself
from time to time. You can do this directly from R using the brilliant [JuliaConnectoR](https://github.com/stefan-m-lenz/JuliaConnectoR) which is the package that **didintrjl** uses to interface with Julia.
...
```{r plots, fig.width=9, fig.height=10, out.width = "100%", dpi=300, fig.align="center"}
...
```{r more_plots_parallel, fig.width=8, fig.height=6, out.width = "100%", dpi=300, fig.align="center"}

plot(res_parallel, ccc = "state")
```

```{r more_plots_event, fig.width=9, fig.height=6, out.width = "100%", dpi=300, fig.align="center"}
